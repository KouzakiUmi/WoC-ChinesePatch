...
**项目说明**：本补丁覆盖全文对白与旁白、界面文本与 54 张改版图片，走 Ren'Py 原生翻译机制，随包附带独立安装器与校验工具。安装过程会先备份原文件，卸载可逐字节完整还原，因此**补丁包本身不携带游戏原始文件**。
...
| 当前版本 | v1.3.10：术语终裁（个人房间）+ HQ 地图整图重制 + 工具修复 |
| 运行要求 | 下载独立 GUI 安装器后无需安装 Python；源码命令行工具需要 Python 3.8+ |

### v1.3.10 更新（2026-09-26）

- **术语终裁**：`personal quarters` → **「个人房间」**（与 HQ 地图标签一致，覆盖早期"个人住所"）；对白 4 行、`review/terms.tsv` 规则与全部报告同步，全篇「个人住所」残留 **0**
- **HQ 地图整图重制**：按最新裁决从英文原版重新生成，六个标签统一文楷风格，替换第三轮的局部合成图（[HQ 整图重制记录](review/image-repair-round4-completion.md)）
- **代词以英文原文为准**：Vivien 的英文原文自身存在代词偏移（紧跟其名 54 he : 4 she），**逐行以原文为准、不做规范化**——回滚 5 行，仅保留 `0020#7620`（原文 `he'd`）
- **工具修复**：`gender_check` 修正两处误报（字面 `\n` 后 `\b` 失效导致 EN 男性代词漏计、"其他/他们"被当作男性代词）→ **冲突 0**
- 全链 `build → compile → manifest 同步 → check` 通过（**payload 哈希 68/68**）

### v1.3.9 更新（2026-09-26，西方奇幻小说规格校对第三轮收口）

**文本 —— 873 行译文修复**（实测去重行数，与 payload 差异行完全对账）

- **结构硬伤**：`0003#1196`、`0021#8162` 的真实换行孤儿行并回主行（游戏内缺行补齐），`0013#4911` 多余括号收敛
- **术语族全部归零**：萨鲁斯、先知、城镇广场、公事房、相位、灵力族、空甲、情谊/牵绊、隧道→地道、personal quarters→**个人房间**（终裁：与 HQ 地图标签一致，覆盖早期"个人住所"）、crew quarters 按场景分工（陆上**成员宿舍** / 船上**船员舱**）
- **逐条修复**：30 份 chunk 工单处理 579 条 findings（改 / 经核不改均留档）+ 3 份尾部工单；70 组"同人同英文不同译"全部统一
- **称谓**：豪尔对君主敬语专项 19 行 → **对君主句含「你」= 0**；码头文书官 `您22/你0`
- **质检**：`scan` 18 条 info（**high/term/low 全 0**）、`fantasy_check` 2 条已知例外、`terms`/`dedupe`/`register` 全 0、断列 0、对白含英文 0 行
...
- `patch_tool check` 新增**逐文件 payload 哈希比对**；`install` 写入前校验 payload 与清单是否一致
- `review_tool` 语域/长度比/重复字/英文残留规则修正，子串与教程误报归零
- 全链 `build → compile → manifest 同步 → check` 通过（**payload 哈希 68/68**）
...
从 v1.3.9 及更早版本升级到 v1.3.10 时，请先用旧版补丁卸载，再安装新版，以清理旧版文件。
...
3. **先备份再写入**：把将覆盖的 56 个原文件复制到 `<游戏目录>\woc_zh_patch_backup\`，随后写入补丁内容。
...
| `game/screens.rpy` | 存档界面动态标签汉化 |
| `game/images/` | 54 张改版图片：书页笔记、区域地图、教程、开场字幕、主菜单等 |
