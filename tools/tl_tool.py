...
def set_paths(tl_dir=None, game_root=None):
    """重定位工作目录与游戏根目录。

    默认 tl_work 与游戏根目录是父子关系; 但本工程已把 tl_work 放进独立仓库,
    此时用 --tl-dir 指定翻译源, 用 --game-root 指定要写 game/tl/chinese 的游戏。
    也可用环境变量 WOC_TL_DIR / WOC_GAME_ROOT。
    """
    global WORK, TEMPLATE, CHUNK_DIALOGUE, CHUNK_STRINGS
    global TARGETS_DIALOGUE, TARGETS_STRINGS, GAMEROOT, TL_DIR
    WORK = os.path.abspath(tl_dir) if tl_dir else HERE
    GAMEROOT = os.path.abspath(game_root) if game_root else os.path.dirname(WORK)
    TEMPLATE = os.path.join(WORK, "template")
    CHUNK_DIALOGUE = os.path.join(WORK, "chunks", "dialogue")
    CHUNK_STRINGS = os.path.join(WORK, "chunks", "strings")
    TARGETS_DIALOGUE = os.path.join(WORK, "targets", "dialogue")
...
def add_path_args(parser):
    parser.add_argument("--tl-dir", help="tl_work 所在目录 (默认与本脚本同目录)")
    parser.add_argument("--game-root", help="游戏根目录, build 会写入其 game/tl/chinese")
...
def main():
    ...
    for p in (ex, ap_, bd, st):
        add_path_args(p)
    args = ap.parse_args()
    set_paths(args.tl_dir or os.environ.get("WOC_TL_DIR"),
              args.game_root or os.environ.get("WOC_GAME_ROOT"))
    if args.cmd == "extract":
